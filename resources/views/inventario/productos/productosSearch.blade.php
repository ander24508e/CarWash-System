...
@if ($productos_buscar->count() == 0)
<div class="alert alert-info mt-3">
    No se encontraron productos con el término: "{{ request()->searchTerm }}"
</div>
@else
    ...
    <div class="card shadow">
        ...
        <div class="card-body p-0">
            <table id="productos-table" class="table table-striped table-bordered">
                ...
                <tbody>
                    @foreach ($productos_buscar as $producto)
                    <tr>
                        <td>{{ $producto->id }}</td>
                        <td>{{ $producto->name_product }}</td>
                        <td>{{ $producto->name_categoria }}</td>
                ...
                <div class="d-flex justify-content-center mt-3">
                    {{ $productos_buscar->links('pagination::bootstrap-4') }}
                </div>
        </div>
    </div>
    <!-- Botón volver -->
